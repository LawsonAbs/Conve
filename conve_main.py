...
#Modified by Sawan Kumar: Save model using dev set score
#
...
def main(args, model_path):
    ...
    for epoch in range(args.epochs):
        ...
        for i, str2var in enumerate(train_batcher):
            ...
            # 根据 e1 ,rel 得到预测的值 pred， 然后用e2_multi计算 损失
        ...
        with torch.no_grad():
            if epoch % 5 == 0 and epoch > 0:
                dev_mrr = ranking_and_hits(model, dev_rank_batcher, vocab, 'dev_evaluation')
...
if __name__ == '__main__':
    ...
    model_path = 'saved_models/{0}_{1}_{2}.model'.format(args.data, model_name, args.model_suffix)
